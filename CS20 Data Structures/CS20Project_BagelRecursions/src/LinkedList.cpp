...
void LinkedList::push_back(Bagels* theData)
{
	if (headptr == NULL)
	{
...
void LinkedList::push_front(Bagels* theData)
{
	ListNode *tempNode = headptr;
...
Bagels* LinkedList::getAt(int nodePlace)
{
	if (headptr == NULL)
	{
		//throw "list empty";
...
Bagels* LinkedList::operator[](const int theIndex)
{
	Bagels* theData;
...
void LinkedList::insertAt(Bagels* theData, int nodePlace)
{
...
void LinkedList::insertSorted(Bagels* theData)
{
	ListNode* tempHead;
	ListNode* newNode = new ListNode;
	newNode->data = theData;
