...
vector<GraphVertex*> Graph::searchDFS(int start) {
	...
	if (this->edgeArray[start].first == nullptr)
	{
	//	cout << "Nothing at given position" << endl;
	}// if nothing at given position
	else
	{
		//Create a Stack<Node> of nodes to visit;
		stack <GraphVertex*> visitStack;
		visitStack.push(this->edgeArray[start].first);
		...
		while(!visitStack.empty())
		{
			...
			else
			{
				...
				for (unsigned int counter = 0; counter < this->getDestVertex(u->Value)->size();
						counter++) {
					//Push w onto the stack;
					visitStack.push(this->getDestVertex(u->Value)->at(counter));
				}// for
...
vector<GraphVertex*> Graph::searchBFS(int start) {
	...
	else
	{
		...
		queue <GraphVertex*> visitQueue;
		visitQueue.push(this->edgeArray[start].first);
...
//		{
			...
			{
				//Pop a node from the stack, let it be u;
				GraphVertex* u = visitQueue.front();
				visitQueue.pop();
				// if (u has been visited) continue;
				if (u->Visited)
				{
				...
				else
				{
					...
					for (unsigned int counter = 0; counter < this->getDestVertex(u->Value)->size();
							counter++) {
						//Push w onto the stack;
						visitQueue.push(this->getDestVertex(u->Value)->at(counter));
					}// for
